...
<div class="container">
	<div class="row no-gutters py-4">
        <div class="col-12 flex-column align-items-center flex-sm-row viewusion-title terapyBg">
            ...
            <div class="flex-column flex-sm-row viewusion-title-content">
                <span class="text-center py-3 pl-3">Терапия</span>
    ...
    <div class="row pb-4">
        <div class="col-6 col-md-4 col-lg-3 pb-4" v-for="(item, index) in viewusionItems" :key="index">
            <categoryItem v-bind:type="'viewusion_terapy'" v-bind:id="item.id" v-bind:title="item.title" v-bind:preview="item.preview"></categoryItem>
        </div>
    </div>
</div>
</template>
...
export default{
    ...
    components: {categoryItem},
     methods: {
        getViewusionItems() {
			var options = {}
			this.$http.get(this.endpoint + 'terapy', options).then((response) => {
